...
OUTPUT_FIELDS = ["发布时间", "帖子名字", "帖子链接", "评论", "匹配关键词", "备注"]
BODY_FIELD = "帖子正文"
OLD_BODY_FIELD = "帖子正文部分"

job.BASE_START = datetime(2026, 7, 26, 0, 0, 0, tzinfo=TZ)
job.BASE_END = datetime.combine(datetime(2026, 7, 31).date(), dt_time(23, 59, 59), tzinfo=TZ)
job.INCREMENTAL_START = datetime(2026, 7, 31, 0, 0, 0, tzinfo=TZ)
job.INCREMENTAL_END = NOW
job.TARGET = None
job.KEEP_FIELDS = OUTPUT_FIELDS

job.OLD_SOURCE_JSON = job.OUT_DIR / "xhs_insforge_incremental_merged_20260725_20260731.json"
job.OUT_JSON = job.OUT_DIR / "xhs_insforge_incremental_merged_20260726_20260801.json"
job.OUT_MD = job.OUT_DIR / "xhs_insforge_incremental_merged_20260726_20260801.md"
job.INBOX_MD = job.INBOX / "小红书建联线索池_新格式_增量合并_20260726-20260801.md"
job.FIELDS_JSON = job.WORK_DIR / "xhs_incremental_merged_base_fields_20260726_20260801.json"
job.RECORDS_JSON = job.WORK_DIR / "xhs_incremental_merged_records_payload_20260726_20260801.json"
job.INC_CACHE_JSON = job.OUT_DIR / "xhs_incremental_candidates_20260731_20260801.json"
job.VERIFY_STATE_JSON = job.WORK_DIR / "xhs_incremental_verify_required_state_20260731_20260801.json"
job.VERIFY_SCREENSHOT = job.WORK_DIR / "xhs_incremental_verify_required_20260731_20260801.png"
...
def hard_excluded(row: dict[str, Any]) -> bool:
    return _original_hard_excluded(row) or job.no_reply_excluded(row) or bool(EXTRA_EXCLUDE.search(_text_blob(row)))
...
def load_old_rows() -> list[dict[str, Any]]:
    ...
    for source_row in source_rows:
        ...
        row = {
            ...
            "备注": str(source_row.get("备注") or "").strip(),
            "note_id": note_id,
...
def merge_rows(old_rows: list[dict[str, Any]], inc_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def add(row: dict[str, Any], prefer: bool = False) -> None:
        ...
        if not existing:
            ...
            return
        for field in ["发布时间", "帖子名字", "帖子链接", "备注"]:
